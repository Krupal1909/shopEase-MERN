...
import { FiPlus, FiEdit, FiTrash2, FiSearch, FiFilter, FiEye, FiUpload } from 'react-icons/fi';
...
const ProductManagement = () => {
  ...
  const handleToggleStatus = async (productId, currentStatus) => {
    try {
      ...
      await adminAPI.bulkUpdateProducts([productId], updates);
...
const ProductModal = ({ isOpen, onClose, onSuccess, product, categories, isEdit = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(isEdit ? 'Error updating product' : 'Error adding product');
